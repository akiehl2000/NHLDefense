---
title: "NHL Stanley Cup Playoffs Sports Reference Scraper"
author: "Adam Kiehl"
date: "7/8/20"
output:
...
map <- map[map %notin% c('WPG/1999', 'WPG/2000', 'WPG/2001', 'WPG/2002', 'WPG/2003', 'WPG/2004', 'WPG/2006', 'WPG/2007', 'WPG/2008', 
                         'WPG/2009', 'WPG/2010', 'WPG/2011')]
...
```{r}
headers <- list()
headers[['skaters_playoffs']] <- c('rank', 'player', 'age', 'pos', 'games', 'goals', 'assists', 'points', 'plus.minus', 
                                   'penalty.mins', 'even.goals','pp.goals', 'pk.goals', 'gw.goals', 'even.assists', 'pp.assists',
...
franchise.map[['team.name']] <- c('Anaheim Ducks', 'Arizona Coyotes', 'Atlanta Thrashers', 'Phoenix Coyotes', 'Boston Bruins',
                                  'Buffalo Sabres', 'Calgary Flames', 'Carolina Hurricanes', 'Chicago Blackhawks', 'Colorado Avalanche',
...
if(!file.exists('./data/team_playoffs.csv')){
  ...
  team_playoffs <- inner_join(team_playoffs, franchise.map, by = c('team' = 'team.name')) %>%
    ...
    write.csv('./data/team_playoffs.csv')
}
```
...
if(!file.exists('./data/goalies_playoffs.csv')){
  ...
  mutate_if(., is.character, str_trim) %>%
  write.csv('./data/goalies_playoffs.csv')
}
```
