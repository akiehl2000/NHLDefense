---
title: "Regular Season Point Share Calculations Script"
author: "Adam Kiehl"
date: "7/8/20"
output:
...
This is a script that utilizes NHL Regular Season data scraped from Hockey Reference (https://www.hockey-reference.com) to calculate point shares for individual players and for teams. Point shares are a system of attributing a player's offensive, defensive, and goalie contributions to overall team points. The system was created by former Vice President of Sports Reference Justin Kubatko (https://www.hockey-reference.com/about/point_shares.html) and was based on Bill James's win shares system. To begin, team, skater, and goalie data is read into R. Note: Scraping script must be run to populate CSVs before this script will execute. 
```{r}
teams <- read_csv('./data/team_reg.csv')
skaters <- read_csv('./data/skaters_reg.csv')
goalies <- read_csv('./data/goalies_reg.csv')
```
...
for (i in 1:nrow(skaters)) {
  ...
  league.points = sum(teams$points[which(teams$year == skaters$year[i])])
  marginal.goals.per.point = c(
...
for (i in 1:nrow(skaters)) {
  goals.created.league = sum(skaters$goals.created[which((skaters$pos2 == skaters$pos2[i]) & (skaters$year == skaters$year[i]))])
  toi.league = sum(skaters$toi[which((skaters$pos2 == skaters$pos2[i]) & (skaters$year == skaters$year[i]))])
  marginal.goals.for = c(
    marginal.goals.for,
...
```{r}
skaters <- skaters %>% select(year, team, player, pos, goals, assists, plus.minus, toi, dps, ops, dps.sr, ops.sr)
goalies <- goalies %>% select(year, team, player, goals, shots, mins, gps, gps.sr)
```
...
```{r}
teams %>% write_csv('./data/team_reg.csv')
skaters %>% write_csv('./data/skaters_reg.csv')
goalies %>% write_csv('./data/goalies_reg.csv')
```